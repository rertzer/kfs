...
static void print_header_mem_infos(int *column_size) {
	printk("%c %-6s %n%c %-6s %n%c %-6s %n%c %-6s %n%c\n",
		VERTICAL_LINE, "pages (in chunks of size n)",	column_size + 0,
...
static void print_row_mem_infos(int *column_size, uint32_t index, mem_info_t *mem_infos) {
	printk("%c %-*u %c %-*d %c %-*d %c %-*d %c\n",
		VERTICAL_LINE, column_size[0] - 3, 1 << index,
		VERTICAL_LINE, column_size[1] - 3, mem_infos[index].free,
		VERTICAL_LINE, column_size[2] - 3, mem_infos[index].used,
		VERTICAL_LINE, column_size[3] - 3, mem_infos[index].total,
		VERTICAL_LINE);
}

static void print_footer_mem_infos(int *column_size, mem_info_t mem_total) {
	printk("%c %*s %c %-*d %c %-*d %c %-*d %c\n",
		VERTICAL_LINE, column_size[0] - 3, "total pages",
		VERTICAL_LINE, column_size[1] - 3, mem_total.free,
		VERTICAL_LINE, column_size[2] - 3, mem_total.used,
		VERTICAL_LINE, column_size[3] - 3, mem_total.total,
		VERTICAL_LINE);
}
