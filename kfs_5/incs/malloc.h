...
#define DEBUG_WHERE                                                                                  \
	(t_debug_where) {                                                                                \
		.function_name = (char*)__func__, .file_name = (char*)__FILE__, .line_number = (int)__LINE__ \
	}
...
#define SET_TEST(test_suite, ptr_index, ...) \
	test(DEBUG_WHERE, test_suite, ptr_index, (t_test_expectation){.diff = {__VA_ARGS__}})

__attribute__((sentinel)) void test_malloc(t_ft_report report, ...);
