...
static void _print_pointer(t_state* state) {
	unsigned int address = va_arg(*state->l_args, unsigned long);
	if (address == 0)
		return ((void)dump_string(state, "(nil)", 5));
...
static void _print_integer(t_state* state) {
	...
	if (state->options.flags & SIGN)
		state->options.flags &= ~BLANK;
	...
	if (state->options.width > len && state->options.flags & ~LEFT) {
		dump_character(state, (state->options.flags & ZERO) ? '0' : ' ', state->options.width - len);
	}
...
static t_fp_dump print_map(char c) {
	switch (c) {
		case 'c':
			return (_print_character);
		case 's':
			return (_print_string);
		case 'p':
			return (_print_pointer);
		case 'i':
			return (_print_integer);
		case 'd':
			return (_print_integer);
		case 'u':
			return (_print_unsigned_integer);
		case 'x':
			return (_print_lower_hexadecimal_case);
		case 'X':
			return (_print_upper_hexadecimal_case);
		case '%':
